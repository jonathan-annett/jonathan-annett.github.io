    ...
    function promised(resolve,reject){
        ...
        .then(function(text1){
            ...
            .then(function(text2){
                ...
                let htmlDiff = diffURLS.htmlDiff ? diffURLS.htmlDiff : (diffURLS.htmlDiff = htmlDiffer() );
                
                let output = htmlDiff(text1, text2);
                let input  = htmlDiff(text2, text1);
...
function showFileDifference(base,newbase,file) {
    ...
    diffURLS(url1, url2) 
      ...
      .then (function(diffs) {
        ...
        document.getElementById("output").innerHTML = diffs.output;
        // document.getElementById("outputOriginal").innerHTML = originalHTML;
        document.getElementById("outputNew").innerHTML = diffs.input;
...
function htmlDiffer() {
        ...
        matching_blocks) {
            ...
            if (match !== null) {
                ...
                if (
                match.end_in_before <= end_in_before && match.end_in_after <= end_in_after) {
                    recursively_find_matching_blocks(
                    before_tokens,
                    after_tokens,
                    index_of_before_locations_in_after_tokens,
        ...
        calculate_operations = function(before_tokens, after_tokens) {
            ...
            for (j = 0, len1 = operations.length; j < len1; j++) {
                op = operations[j];
                if (
                (is_single_whitespace(op) && last_op.action === "replace") || (op.action === "replace" && last_op.action === "replace")) {
