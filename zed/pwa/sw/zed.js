...
/* --args-->*/  ){
    ...
    function pageCode(msg,resolve,reject,reply) {
        ...
        let { title,url,urlPostfix } = msg,
            ...
            windowOpts = {
                url    : '/zed?url=' + encodeURIComponent(url) + '&title=' + encodeURIComponent(title) + urlPostfix,
                title  : title,
                left   : 0,
                top    : 0, 
                width  : 1024,
        ...
        wTools.promise.open (windowOpts).then(resolve).catch(reject);
...
/* --args-->*/  ){
    ...
    function pageCode(msg,resolve,reject,reply) {
        ...
        let windowOpts = {
                url    : '/zed?url=' + encodeURIComponent(url) + '&title=' + encodeURIComponent(title),
                title  : title,
                left   : 0,
                top    : 0, 
                width  : 1024,
...
/* --args-->*/  ){
    ...
    function pageCode(msg,resolve,reject,reply) {
        ...
        const proj = wTools.getWindowForURL(url);
        if (proj) {
            var win = openProjects[url].win;
            win.focus();
            const zed = (win.contentWindow||win).zed;
            if (zed) {
                zed.services.editor.getActiveEditor().focus();
            }
        } else {
